  ...
  statement {
    ...
    resources = flatten([
      ...
      each.key == "all" || endswith(each.key, account.env_name) ? [
        ...
        "arn:aws:glue:${var.region}:${account.account_id}:database/${startswith(each.value, "unified_") ? "*" : each.value}",
        "arn:aws:glue:${var.region}:${account.account_id}:table/${startswith(each.value, "unified_") ? "*" : each.value}/*"
      ] : []
    ])
  }
}
